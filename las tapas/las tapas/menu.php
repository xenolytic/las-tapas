...
<div id="orderModal">
    <div style="padding:20px; border-radius:5px; text-align:center;">
        ...
        <br><br>
        <label for="tableNumber" style="color: #000;">Table Number:</label>
        <input type="text" id="tableNumber" placeholder="Enter table number" style="width:150px; text-align:center;">
        <br><br>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        window.confirmOrder = function() {
            ...
            addToCart(item, price, quantity, tableNumber); // Send order to the server
            closeOrderModal(); // Hide the modal after confirmation
        }

        window.addToCart = function(item, price, quantity, tableNumber) {
            ...
            xhr.send("item=" + encodeURIComponent(item) + 
                     "&price=" + price + 
                     "&quantity=" + quantity + 
                     "&table_number=" + encodeURIComponent(tableNumber)); // Corrected the name to `table_number`
        }
    });
</script>
